      ...
      <form onSubmit={handleSubmit}>
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-colors"
          />
          {query && (
            <button
              type="button"
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 overflow-hidden">
          {results.map((article) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-semibold text-teal-600">{article.category}</span>
          ...
          <Link
            ...
            onClick={() => { setIsOpen(false); onClose?.() }}
            className="block px-4 py-2.5 text-center text-sm font-medium text-teal-600 bg-gray-50 hover:bg-gray-100 transition-colors"
          >
